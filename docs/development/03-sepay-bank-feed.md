...
1. `POST /integrations/sepay/connect` tạo connection draft: workspace/account target, nonce/state, requested read scopes, expiry.
2. Redirect user tới Hosted Link/OAuth; callback kiểm state, PKCE và actor/workspace server-side.
...
1. Resolve `bank_connection` bằng provider account mapping; event không map được → quarantined + alert, không đoán workspace.
...
Điều kiện transfer match tối thiểu: account khác cùng workspace, chiều đối lập, same currency/amount và window thời gian cấu hình (ví dụ ±2 ngày); nếu nhiều candidate hoặc confidence thấp thì tạo review, không tự ghép. Auto expense có `source=bank_feed`, `classification=auto`, `posted_transaction_id` và audit.
...
`bank_automation_rules` fields: `id`, `workspace_id`, nullable `account_id`, `priority`, `direction`, `content_pattern`, `reference_pattern`, `min_amount`, `max_amount`, `recurrence_hint`, `action_type`, `category_id`, `enabled`, `version`, `created_by`.

- Sort account rule trước workspace rule, sau đó priority giảm dần và updated time tăng dần.
...
| Unknown connection | quarantine event | investigate mapping; do not assign workspace |
