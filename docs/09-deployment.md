...
Biến cấu hình cần có: `DATABASE_URL`, `JWT_ISSUER`, `JWT_AUDIENCE`, `OBJECT_STORAGE_*`, `APP_BASE_URL`, `ENCRYPTION_KEY`, `SMTP_*`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, `ASSISTANT_APPROVAL_TTL`, `HERMES_EXECUTOR_CA` và executor credential. Không commit secrets vào repository.
